...
/**
 扫码区域动画效果
 */
typedef NS_ENUM(NSInteger,ScanViewAnimationStyle)
{
    ScanViewAnimationStyle_LineMove,   //线条上下移动
    ScanViewAnimationStyle_NetGrid,//网格
    ScanViewAnimationStyle_LineStill,//线条停止在扫码区域中央
    ScanViewAnimationStyle_None    //无动画
...
/**
 扫码区域4个角位置类型
 */
typedef NS_ENUM(NSInteger, ScanViewPhotoframeAngleStyle)
{
    ScanViewPhotoframeAngleStyle_Inner,//内嵌，一般不显示矩形框情况下
    ScanViewPhotoframeAngleStyle_Outer,//外嵌,包围在矩形框的4个角
    ScanViewPhotoframeAngleStyle_On   //在矩形框的4个角上，覆盖
};
...
/**
 @brief  扫码区域的4个角类型
 */
@property (nonatomic, assign) ScanViewPhotoframeAngleStyle photoframeAngleStyle;
...
/**
 @brief  扫码动画效果:线条或网格
 */
@property (nonatomic, assign) ScanViewAnimationStyle anmiationStyle;
